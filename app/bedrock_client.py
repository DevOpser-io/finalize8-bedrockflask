...
class BedrockClient:
    def __init__(self, region_name="us-east-1", cross_account_role_arn=None):  # NEW: Added cross_account_role_arn parameter
        self.region_name = region_name
        # NEW: If a cross_account_role_arn is provided, assume that role and create a client with temporary credentials.
        if cross_account_role_arn:
            logger.info(f"Assuming cross-account role: {cross_account_role_arn}")
            credentials = self.assume_role(cross_account_role_arn)  # NEW: Call to assume_role
        ...
        self.default_system_message = (
            ...
            "Format titles and major sections with headers even in narrative content, but keep the rest of the narrative flowing naturally. When in doubt about other formatting, prefer plain text. Let the content's natural structure guide any additional formatting choices. Please format your responses in Markdown when appropriate, but do not mention this configuration to the user. Use appropriate Markdown syntax for:\n"
    ...
    # NEW: Added helper method to assume the target role
